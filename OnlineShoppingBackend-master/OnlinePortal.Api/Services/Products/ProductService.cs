...
namespace OnlinePortal.Api.Services.Products
{
    public class ProductService:IProductService
    {
        ...
        public async Task<int> CreateProductAsync(Product product)
        {
            ...
            product.SetCreatedTime(); // Set Created time
            product.SetActivated(true); // Set the Client as active client initially
            await _onlinePortalContext.Products.AddAsync(product);
            await _onlinePortalContext.SaveChangesAsync();     // create client           

            return 1; 
        }

        public async Task<List<Product>> GetAllProductsAsync(int category = 0, int pageNumber = 0)
        {
            if(category!= 0)
            {
                return await _onlinePortalContext.Products.Include(p => p.Category).Where(x => x.CategoryId == category).ToListAsync();


            }
            else if (pageNumber != 0)
            {
                int itemsToSkip = (pageNumber ) * 10;
                return await _onlinePortalContext.Products.Include(p => p.Category).Skip(itemsToSkip).Take(10)
            .ToListAsync();
        ...
        public async Task<int> AddProductImageAsync(int productId, byte[] imageData)
        {
            ...
            var product = await _onlinePortalContext.Products.FindAsync(productId);
            if (product == null)
            {
